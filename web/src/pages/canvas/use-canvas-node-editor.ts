...
import { exportCanvasMedia } from "@/services/desktop-media-export";
...
export function useCanvasNodeEditor({
    ...
    const downloadNodeImage = useCallback((node: CanvasNodeData) => {
        ...
        void exportCanvasMedia(node, canvasTitle)
            .then((result) => {
                if (result.status === "exported") message.success(result.path ? `已导出到 ${result.path}` : "媒体已导出");
            })
            .catch((error) => message.error(error instanceof Error ? error.message : "媒体导出失败"));
    }, [canvasTitle, message]);
