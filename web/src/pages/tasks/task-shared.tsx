import { CONTENT_MODERATION_ERROR_CODE, explainGenerationError, isContentModerationError, shouldBlockAutomaticRetry } from "@/lib/generation-error";
...
export function taskAttentionReason(task: GenerationTask) {
    ...
    const explanation = explainGenerationError(task.error || task.errorCode, { taskId: task.id, providerRequestId: task.providerRequestId, model: task.model, createdAt: task.createdAt, stage: task.stage });
...
export function taskRetryBlocked(task: GenerationTask) {
    return shouldBlockAutomaticRetry(task.error || task.errorCode, task.stage) || task.errorCode === CONTENT_MODERATION_ERROR_CODE || isContentModerationError(task.error);
}
