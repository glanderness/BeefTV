...
export async function readFetchError(response: Response, fallback: string) {
    ...
    try {
        return explainGenerationError({ status: response.status, data: JSON.parse(text), message: fallback }).message;
    } catch {
        return explainGenerationError({ status: response.status, data: text, message: fallback }).message;
    }
}
