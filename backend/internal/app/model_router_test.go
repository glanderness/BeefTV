...
func TestImageVariantsMatchResolutionAndActualReferences(t *testing.T) {
	...
	for _, operation := range []string{"", "image", "text_to_image", "image_to_image"} {
		for _, quality := range []string{"1K", "2K", "4K"} {
			for _, imageCount := range []int{0, 1, 3} {
				...
				tier := channelModelVariantForIntent(channelModel, intent)
	...
	if tier := channelModelVariantForIntent(channelModel, ModelRequestIntent{Capability: "image", Options: map[string]any{"quality": "8k"}}); tier != nil {
